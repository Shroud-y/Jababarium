package jababarium.expand.entities;
...
public class Carrier extends JBBaseEntity implements Teamc, Rotc, Scaled {
        ...
        if (!onMove && unit != null) {
            ...
            if ((contained && !Units.canCreate(team, unit.type)) || (surviveTime > 0)) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    Draw.rect(JBContent.pointerRegion, x + Tmp.v1.x, y + Tmp.v1.y, size, size, i * 90 - 90f);
                }
    ...
    public void update() {
        ...
        if (onMove && contained) {
            ...
            JBGroups.gravityFields.intersect(x - drawSize / 2f, y - drawSize / 2f, drawSize, drawSize, b -> {
                if (b.owner != team) {
                    intercepted = true;
                    toCarry.unit.damage(toCarry.unit.health * 0.3f);
                    PosLightning.createEffect(b, this, b.owner.color, 2, PosLightning.WIDTH);
                    JBFx.square45_4_45.at(x, y, team.color);
                }
            });
        }
