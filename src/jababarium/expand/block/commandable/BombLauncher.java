package jababarium.expand.block.commandable;
...
public class BombLauncher extends CommandableAttackerBlock {
    ...
    public void init() {
        super.init();
        if (bullet.shootEffect == JBFx.boolSelector)
            bullet.shootEffect = JBFx.square(baseColor, 50f, 6, size * tilesize * 2f, size);
    }
    ...
    public class BombLauncherBuild extends CommandableAttackerBlockBuild {
        ...
        public void draw() {
            ...
            Draw.draw(Draw.z(), () -> {
                if (reload / reloadTime > 1) {
                    ...
                    Drawf.construct(x, y, bombRegion, baseColor, 0, reload / reloadTime,
                            Mathf.curve(1 - reload / reloadTime, 0, 0.15f) * warmup, totalProgress);
            });
        }
        ...
        public void shoot(Vec2 target) {
            ...
            bomb.init(team, 80f, this, target.x + rand.range(spread) + Mathf.range(12f),
                    target.y + rand.range(spread) + Mathf.range(12f), true);
            bomb.add();
        }
    }
    ...
    public class BombEntity extends CommandEntity implements Damagec {
        ...
        public float cx() {
            return x + (parent ? fin(Interp.pow2In) : fout(Interp.pow2Out))
                    * (floatX + Mathf.randomSeedRange(id() + 3, floatX));
        }

        public float cy() {
            return y + (parent ? fin(Interp.pow2In) * 1.25f : fout(Interp.pow5Out))
                    * (floatY + Mathf.randomSeedRange(id() + 2, floatY));
        }
        ...
        public void draw() {
            ...
            if (parent) {
                ...
                Fill.light(cx, cy, 10, 25f * (rad + scale - 1f), Tmp.c2.set(Pal.engine).a(alpha),
            ...
            Tmp.v1.trns(225, (parent ? fin(Interp.pow2In) * 1.25f : fout(Interp.pow5Out))
                    * (floatY + Mathf.randomSeedRange(id() + 2, floatY)));
        ...
        public void update() {
            ...
            trail.update(cx(), cy());
            if (Mathf.chance(bullet.trailChance))
                bullet.trailEffect.at(cx(), cy(), bullet.trailParam, baseColor);
            if (time >= lifetime) {
                remove();
            }
        }
